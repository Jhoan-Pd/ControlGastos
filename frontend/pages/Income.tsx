...
export default function Income() {
  ...
  const [form, setForm] = useState({ monto: '', fecha: hoy, categoriaId: '', fuenteIngreso: '' });

  const cargar = async () => {
    if (!usuario) return;
    try {
      const [rIngresos, rCats] = await Promise.all([
        api.get(`/ingresos/usuario/${usuario.id}`),
        api.get('/categorias'),
      ]);
      setIngresos(rIngresos.data);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await api.post('/ingresos', {
        ...
        fecha: form.fecha,
        esActiva: true,
        fuenteIngreso: form.fuenteIngreso,
        categoriaId: form.categoriaId ? parseInt(form.categoriaId) : null,
      });
      setForm({ monto: '', fecha: hoy, categoriaId: '', fuenteIngreso: '' });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        >
          ...
          {mostrarForm ? 'Cancelar' : 'Agregar ingreso'}
        </button>
      </div>
      ...
      {mostrarForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Fuente del ingreso</label>
            <input type="text"
              value={form.fuenteIngreso} onChange={(e) => setForm({ ...form, fuenteIngreso: e.target.value })}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
        ...
        {loading ? (
          ...
          <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-gray-50">
              <tr>
                {['Fecha', 'Monto', 'Fuente', 'Categoría'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {ingresos.length === 0 ? (
                <tr><td colSpan={4} className="px-5 py-10 text-center text-gray-400 text-sm">No hay ingresos registrados</td></tr>
              ) : ingresos.map((i) => (
                <tr key={i.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3 text-sm text-gray-700">{format(new Date(i.fecha + 'T00:00:00'), 'dd/MM/yyyy')}</td>
                  <td className="px-5 py-3 text-sm font-semibold text-green-600">{fmt(i.monto)}</td>
                  <td className="px-5 py-3 text-sm text-gray-600">{i.fuenteIngreso || <span className="text-gray-400">—</span>}</td>
                  <td className="px-5 py-3 text-sm text-gray-600">{i.categoria?.nombre ?? <span className="text-gray-400">Sin categoría</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
